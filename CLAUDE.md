...
- **Surfaces M5 (Ember E2) SHIPPED 2026-07-20** — the portal 🗺 Table view is interactive: players drag their OWN token only (server-enforced via `characterId`, grid-snap, atomic save), 📍 ping (ephemeral SSE relay to all browsers + presenter + DM map editors, token color + name; DM Alt+click pings reach phones too), 📏 measure (local, ft). New portal routes `POST /api/table/move-token` + `/api/table/ping`; tool pills in `PlayerApp.tsx`, interactive `PresenterMap` in `MapEditor.tsx`.
- **Surfaces M3+M4 SHIPPED 2026-07-16** — M3: ⚔ Table + 🛡 Party are real Electron windows. M4: structured inventory (rows + attunement + charge pips), auto-AC (`src/shared/inventory.ts`: hand-tabled SRD armors, `effectiveAc()` everywhere), coin pouches with make-change, 🎒 party stash (`<campaign>/party.json`, transfer-never-copy + log, portal drawer), catalog-search Grant. Free-text `equipment` auto-migrates on load (originals kept in `legacyEquipment`).
