...
function MainApp() {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (s.libraryOpen || s.triage || s.discordOpen || s.switcherOpen || s.captureOpen || s.helpOpen || s.compendiumOpen || s.mapEditorOpen) return
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (k !== 'k' && k !== 'j') return
      const s = useStore.getState()
      if (s.libraryOpen || s.triage || s.discordOpen || s.compendiumOpen || s.mapEditorOpen) return
      e.preventDefault()
      e.stopPropagation()
      if (k === 'k') {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      const s = useStore.getState()
      if (s.libraryOpen || s.triage || s.discordOpen || s.switcherOpen || s.captureOpen || s.compendiumOpen || s.mapEditorOpen) return
      e.preventDefault()
